...
export class ProductBrandAdd extends Component {
    constructor(props) {
        super(props);
        this.state = {
            product : '',
    ...
    async componentDidMount(){
        try{
            const response = await axios.get(`/seeProductBrands/${localStorage.getItem('slno')}`, {
                headers : {
    ...
    delType = async (e, Prdsln)=>{
        try{
            const response = await axios.get(`/delProdBrand/${localStorage.getItem('slno')}/${Prdsln}`,{
                headers : {
                    'Content-Type' : 'application/json'
                }
            });
